...
描述：亿境自动化平台的 API Key。目前用于 YJTest MCP 服务，可以让大模型拥有查询、保存、执行等能力。

注意！！！

如果 YJTest 平台部署外网访问，强烈建议修改这个默认的key！！！

修改后记得同步更新 YJTest_MCP 目录下的 YJTest_tools.py 中对应的 key。
...
YJTest-Tools：使大模型拥有操控亿境自动化平台的能力。
...
1、上传YJTest_Skills目录下的俩个zip文件即可使用。
