...
### 1. 默认API Key
- **Key值**: `yjtest-default-mcp-key-2026`
- **用途**: MCP服务访问后端API

### 2. 默认MCP配置
系统自动添加两个MCP工具配置：
- **YJTest-Tools** (http://mcp:8006) - 测试用例管理
...
| MCP服务 | http://localhost:8914 | YJTest MCP工具 |
...
```bash
# 测试API Key是否有效
curl -H "X-API-Key: yjtest-default-mcp-key-2026" \
