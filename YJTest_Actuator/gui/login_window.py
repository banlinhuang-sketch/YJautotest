...
class LoginWindow(QDialog):
    ...
    def _init_ui(self):
        """初始化界面 - 左右分栏设计"""
        self.setWindowTitle("YJTest 执行器登录")
    ...
    def _get_window_icon(self) -> Optional[QIcon]:
        ...
        icon_paths = [
            base_path / "data" / "app-icon.svg",
            base_path / "app-icon.svg",
            base_path.parent / "YJTest_Vue" / "public" / "app-icon.svg",
        ]
    ...
    def _create_logo(self) -> QPixmap:
        ...
        # 尝试加载项目 logo 图片
        logo_paths = [
            Path(__file__).parent.parent / "data" / "app-icon.svg",
            Path(__file__).parent.parent.parent / "YJTest_Vue" / "public" / "app-icon.svg",
            Path(__file__).parent.parent / "app-icon.svg",
        ]
