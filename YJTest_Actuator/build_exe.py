...
生成文件:
    dist/YJTest_Actuator/YJTest_Actuator.exe
"""
...
def build():
    ...
    # 复制配置文件
    print("\n[3/3] 复制配置文件...")
    dist_dir = Path('dist/YJTest_Actuator')
    ...
    print("\n使用说明:")
    print("  1. 进入 dist/YJTest_Actuator/ 目录")
    print("  2. 编辑 config.toml 配置文件")
    print("  3. 运行 YJTest_Actuator.exe 或 start.bat")
    print("\n首次运行说明:")
...
chcp 65001 >nul
title YJTest Actuator

echo ========================================
echo   YJTest UI 自动化执行器
echo ========================================
echo.
...
chcp 65001 >nul
title YJTest Actuator (无GUI模式)

echo ========================================
echo   YJTest UI 自动化执行器 (无GUI模式)
echo ========================================
echo.

REM 启动执行器（无 GUI 模式，使用配置文件中的账号密码）
YJTest_Actuator.exe --no-gui
