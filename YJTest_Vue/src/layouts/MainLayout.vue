    ...
    <a-layout-header class="header">
      <div class="left-section">
        <div class="logo" unselectable="on">
          <img :src="brandLogoUrl" :alt="`${brandName} Logo`" class="logo-icon" />
          <div class="logo-copy">
            <span class="logo-text">{{ brandShortName }}</span>
            <span class="logo-subtext">{{ brandName }}</span>
          </div>
        </div>
...
import { brandName, brandReleaseUrl, brandShortName } from '@/config/brand';
...
const releasePageUrl = computed(() => versionInfo.value?.releaseUrl || (brandReleaseUrl ? `${brandReleaseUrl}/releases` : undefined));
...
.main-layout {
  background:
    ...
    radial-gradient(circle at 88% 4%, rgba(167, 210, 255, 0.5), transparent 28%),
    linear-gradient(180deg, #f7faff 0%, #eef3fb 46%, #e6edf7 100%);
}
