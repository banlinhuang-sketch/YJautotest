...
const isPermission = (value: unknown): value is Permission => {
  ...
  return typeof permission.id === 'number'
    && typeof permission.codename === 'string'
    && !!permission.content_type
    && typeof permission.content_type === 'object';
};

const normalizePermissionListPayload = (
  payload: unknown,
): { data: Permission[]; total: number; statusCode?: number } | null => {
  if (Array.isArray(payload) && payload.every(isPermission)) {
    return {
      data: payload,
  ...
  if (Array.isArray(responseData.data) && responseData.data.every(isPermission)) {
    return {
      data: responseData.data,
      total: typeof responseData.total === 'number' ? responseData.total : responseData.data.length,
      statusCode: typeof responseData.code === 'number' ? responseData.code : undefined,
    };
  }

  if (Array.isArray(responseData.results) && responseData.results.every(isPermission)) {
    return {
      data: responseData.results,
      total: typeof responseData.count === 'number' ? responseData.count : responseData.results.length,
    };
  }
...
const normalizePermissionDetailPayload = (payload: unknown): Permission | null => {
  if (isPermission(payload)) {
    return payload;
  }
  ...
  const responseData = payload as Record<string, unknown>;
  if (isPermission(responseData.data)) {
...
export const getPermissionList = async (params?: PaginationParams): Promise<PermissionListResponse> => {
  ...
  try {
    ...
    const normalizedPayload = normalizePermissionListPayload(response.data);
    if (normalizedPayload) {
      return {
        success: true,
        data: normalizedPayload.data,
        statusCode: normalizedPayload.statusCode,
...
export const getPermissionDetail = async (permissionId: number): Promise<{ success: boolean; data?: Permission; error?: string }> => {
  ...
  try {
    ...
    const normalizedPermission = normalizePermissionDetailPayload(response.data);
    if (normalizedPermission) {
      return {
        success: true,
        data: normalizedPermission,
      };
    } else {
      return {
        success: false,
