...
YJTest 是一个基于 Django REST Framework 构建的 AI 驱动测试自动化平台，核心功能是通过 AI 智能生成测试用例。平台集成了 LangChain、MCP（Model Context Protocol）工具调用、项目管理、测试用例管理以及先进的知识库管理和文档理解功能。利用大语言模型和多种嵌入服务（OpenAI、Azure OpenAI、Ollama 等）的能力，自动化生成高质量的测试用例，并结合知识库提供更精准的测试辅助，为测试团队提供一个完整的智能测试管理解决方案。
...
| `USER_AGENT` | 否 | 默认值 | `YJTest-Django/1.0 (Yijing Automation Platform)` | HTTP 请求的用户代理标识 |
...
# 使用 .env 文件运行
docker run -d -p 8000:8000 --env-file .env yjtest-django
```
...
uvicorn yjtest_django.asgi:application --host 0.0.0.0 --port 8000 --workers 4
```
...
**YJTest** - AI驱动测试用例生成，让测试更智能，让开发更高效！
