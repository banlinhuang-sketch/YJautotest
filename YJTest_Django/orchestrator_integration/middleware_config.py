...
import logging
import re
from typing import Callable, List, Optional, Dict, Any, Iterable, Sequence
...
def _extract_tool_name_from_definition(tool: Any) -> Optional[str]:
...
def _normalize_requested_tool_name(
    requested_name: str, available_tool_names: Optional[Sequence[str]]
) -> tuple[str, bool]:
    normalized_name = (requested_name or "").strip()
    if not normalized_name or not available_tool_names:
        return normalized_name, False

    available = [
        str(tool_name).strip() for tool_name in available_tool_names if str(tool_name).strip()
    ]
    if not available or normalized_name in available:
        return normalized_name, False

    candidates: List[str] = []
    for tool_name in available:
        if len(normalized_name) <= len(tool_name):
            continue
        if len(normalized_name) % len(tool_name) != 0:
            continue

        repeat_count = len(normalized_name) // len(tool_name)
        if repeat_count >= 2 and tool_name * repeat_count == normalized_name:
...
def normalize_tool_call_names(
    ...
    available_tool_names: Optional[Sequence[str]],
    ...
    for tool_call in tool_calls or []:
        ...
        normalized_name, changed = _normalize_requested_tool_name(
            requested_name, available_tool_names
        )
        if not changed:
            normalized_calls.append(tool_call)
            continue

        normalized_calls.append({**tool_call, "name": normalized_name})
        changed_count += 1

    return normalized_calls, changed_count


def _clone_ai_message_with_tool_calls(
    message: AIMessage, tool_calls: List[Dict[str, Any]]
) -> AIMessage:
    try:
...
class ToolCallNameNormalizationMiddleware(AgentMiddleware):
    """Normalize repeated tool names like `foofoofoo` back to `foo`."""

    def __init__(self) -> None:
        self.tools = []

    @staticmethod
    def _available_tool_names(request: ModelRequest) -> List[str]:
        names: List[str] = []
        for tool in request.tools or []:
            tool_name = _extract_tool_name_from_definition(tool)
            if tool_name:
                names.append(tool_name)
        return names

    def _normalize_model_result(
        self, request: ModelRequest, result: ModelResponse | AIMessage | Any
    ) -> ModelResponse | AIMessage | Any:
        available_tool_names = self._available_tool_names(request)
        if not available_tool_names:
            return result

        def _normalize_ai_message(message: AIMessage) -> tuple[AIMessage, int]:
            ...
            if changed_count == 0:
                return message, 0
            return _clone_ai_message_with_tool_calls(message, normalized_tool_calls), changed_count

        if isinstance(result, AIMessage):
            normalized_message, changed_count = _normalize_ai_message(result)
            if changed_count:
                logger.warning(
                    "ToolCallNameNormalizationMiddleware: normalized %d repeated tool call name(s) for model output",
                    changed_count,
                )
        ...
        for message in result.result:
            if isinstance(message, AIMessage) and getattr(message, "tool_calls", None):
                normalized_message, message_changed_count = _normalize_ai_message(message)
                normalized_messages.append(normalized_message)
                changed_count += message_changed_count
            else:
        ...
        logger.warning(
            "ToolCallNameNormalizationMiddleware: normalized %d repeated tool call name(s) across %d model message(s)",
            changed_count,
    ...
    def wrap_model_call(
        ...
        return self._normalize_model_result(request, handler(request))
    ...
    async def awrap_model_call(
        ...
        return self._normalize_model_result(request, await handler(request))
...
def get_standard_middleware(
    ...
    middleware.append(ToolCallNameNormalizationMiddleware())
    logger.debug("已添加 ToolCallNameNormalizationMiddleware")
