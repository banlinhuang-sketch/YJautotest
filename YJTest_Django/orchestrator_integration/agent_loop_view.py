...
from yjtest_django.checkpointer import get_async_checkpointer
...
def _is_multimodal_human_content(content: Any) -> bool:
...
def _stringify_message_content(content: Any) -> str:
    ...
    if isinstance(content, list):
        ...
        if image_count:
            text_parts.append(f"[{image_count} image input(s) omitted for text-only model]")
...
def _clone_message_with_content(msg: Any, content: str) -> Any:
    return _clone_message(msg, content=content)


def _clone_message(msg: Any, **updates: Any) -> Any:
    try:
...
def _normalize_message_for_string_only_model(
    msg: Any, allow_multimodal_human: bool
) -> tuple[Any, bool]:
    content = getattr(msg, "content", None)
    if isinstance(content, str):
        return msg, False

    if allow_multimodal_human and isinstance(msg, HumanMessage):
        if _is_multimodal_human_content(content):
            return msg, False

    normalized_content = _stringify_message_content(content)
    return _clone_message_with_content(msg, normalized_content), True


def _normalize_tool_result_for_model(tool: BaseTool, result: Any) -> Any:
    ...
    if response_format == "content_and_artifact":
        ...
        normalized_content = _stringify_message_content(content)
        if artifact is None and not isinstance(content, str):
            artifact = content
        return normalized_content, artifact

    if isinstance(result, str):
        return result

    return _stringify_message_content(result)


def _wrap_tool_for_string_tool_messages(tool: BaseTool) -> BaseTool:
    ...
    async def wrapped_coroutine(*args, **kwargs):
        ...
        return _normalize_tool_result_for_model(tool, result)
    ...
    update_payload = {}
    if original_coroutine is not None:
        update_payload["coroutine"] = wrapped_coroutine
    if original_func is not None:
        update_payload["func"] = wrapped_func

    try:
        return tool.model_copy(update=update_payload)
    except Exception:
        wrapped_tool = copy.deepcopy(tool)
        if "coroutine" in update_payload:
            wrapped_tool.coroutine = wrapped_coroutine
        if "func" in update_payload:
            wrapped_tool.func = wrapped_func
        return wrapped_tool


def _wrap_tools_for_string_tool_messages(tools: List[BaseTool]) -> List[BaseTool]:
    return [_wrap_tool_for_string_tool_messages(tool) for tool in tools]


def _build_sanitized_messages(
    messages: List[Any],
    allow_multimodal_human: bool = False,
    ...
    for msg in messages:
        ...
        if isinstance(msg, AIMessage) and getattr(msg, "tool_calls", None):
            ...
            if changed_count:
                msg = _clone_message(msg, tool_calls=normalized_tool_calls)
                fix_count += changed_count
        ...
        if isinstance(msg, ToolMessage):
            ...
            if tc_id in pending_call_ids:
                ...
                normalized_tool_name = pending_call_names.get(tc_id) or getattr(
                    msg, "name", None
                )
                if _is_tool_content_problematic(msg):
                    result.append(
                        ToolMessage(
                            ...
                            tool_call_id=tc_id,
                            name=normalized_tool_name or "unknown",
                        )
                    )
                    fix_count += 1
                else:
                    if (
                        ...
                        and getattr(msg, "name", None) != normalized_tool_name
                    ):
                        msg = _clone_message(msg, name=normalized_tool_name)
                        fix_count += 1
                    result.append(msg)
            else:
                # 悬空 ToolMessage，丢弃
        ...
        # 其他消息类型
        _flush_pending()
        normalized_msg, changed = _normalize_message_for_string_only_model(
            msg, allow_multimodal_human=allow_multimodal_human
        )
        if changed:
            fix_count += 1
...
async def _sanitize_history_before_model_call(
    ...
    clean_msgs, fix_count = _build_sanitized_messages(
        messages,
        allow_multimodal_human=allow_multimodal_human,
    ...
    async def _create_stream_generator(
        ...
        try:
            ...
            async with get_async_checkpointer() as checkpointer:
                ...
                tools = _wrap_tools_for_string_tool_messages(tools)
                ...
                await _sanitize_history_before_model_call(
                    ...
                    allow_multimodal_human=bool(active_config.supports_vision),
                                    ...
                                    if node_name == "agent" and isinstance(
                                        ...
                                        for msg in messages:
                                            if (
                                                ...
                                                # 新的工具调用 -> 新步骤开始
                                                step_count += 1
                                                current_tool_calls = normalized_tool_calls
                                                tool_names_in_step = [
                                                    tc.get("name", "unknown")
                                                    if isinstance(tc, dict)
                                        ...
                                        for tool_msg in tool_messages:
                                            if hasattr(tool_msg, "content"):
                                                raw_tool_output = getattr(
                                                    tool_msg, "artifact", None
                                                )
                                                content = (
                                                    raw_tool_output
