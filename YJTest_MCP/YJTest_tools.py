...
# mcp 初始化
mcp = FastMCP(name="YJTest_tools")

# 从环境变量读取后端地址
# 默认使用 Docker 网络中的 backend 服务名称
base_url = os.getenv("YJTEST_BACKEND_URL") or os.getenv("WHARTTEST_BACKEND_URL", "http://backend:8000")

# 从环境变量读取 API Key
# 请在 .env 文件或环境变量中设置 YJTEST_API_KEY
api_key = os.getenv("YJTEST_API_KEY") or os.getenv("WHARTTEST_API_KEY", "")
...
@mcp.tool(description="获取YJTest平台项目的名称和对应id")
def get_project_name_and_id() -> str:
    """获取YJTest平台项目的名称和对应id"""
    ...
    try:
        ...
        error_info = {
            ...
            "url": url,
            "base_url": base_url,
            "suggestion": "请检查后端服务是否启动，或检查 YJTEST_BACKEND_URL 环境变量配置",
        }
        return json.dumps(error_info, indent=4, ensure_ascii=False)
...
@mcp.tool(description="根据YJTest平台项目id去获取模块及id")
def module_to_which_it_belongs(project_id: int) -> str:
    """根据YJTest平台项目id去获取模块及id"""
...
@mcp.tool(description="获取YJTest平台用例等级")
...
@mcp.tool(description="获取YJTest平台测试类型")
...
@mcp.tool(description="获取YJTest平台用例名称和对应id")
...
@mcp.tool(description="获取YJTest平台用例详情")
...
@mcp.tool(description="YJTest平台保存操作截图到对应用例中")
...
@mcp.tool(description="保存YJTest平台功能测试用例")
...
@mcp.tool(description="编辑YJTest平台功能测试用例")
